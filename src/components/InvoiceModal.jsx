      ...
      <div className={`glass-bespoke shadow-[0_40px_80px_rgba(21,33,54,0.15)] rounded-[3rem] overflow-hidden relative z-10 w-full max-w-4xl max-h-[90vh] flex flex-col transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] transform ${isOpen ? 'scale-100 translate-y-0 opacity-100' : 'scale-95 translate-y-8 opacity-0'}`}>
        ...
        <div className="px-6 sm:px-10 pt-8 sm:pt-10 pb-6 flex flex-col sm:flex-row justify-between items-start gap-6 sm:gap-0 relative z-10 shrink-0 border-b border-gray-100/50">
          ...
          <div className="text-right sm:text-left flex flex-col items-start sm:items-end w-full sm:w-auto border-t border-gray-100/50 sm:border-0 pt-4 sm:pt-0">
            ...
            <p className="text-xl sm:text-3xl font-black text-navy font-sans tracking-tighter">#INV-2023-089</p>
          </div>
        </div>
          ...
          <div className="p-6 sm:p-10 pt-6 sm:pt-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-8 sm:mb-10">
              <div className="bg-white/40 rounded-[2rem] p-6 sm:p-8 border border-white/50">
                <p className="text-teal text-[10px] font-black uppercase tracking-[0.2em] mb-3 sm:mb-4">Client Details</p>
                <p className="text-xl sm:text-2xl font-black text-navy mb-1">أحمد عبدالله</p>
                <p className="text-gray-500 font-sans font-bold tracking-wide" dir="ltr">+966 50 123 4567</p>
              </div>
              <div className="bg-white/40 rounded-[2rem] p-6 sm:p-8 border border-white/50">
                <p className="text-teal text-[10px] font-black uppercase tracking-[0.2em] mb-3 sm:mb-4">Vehicle Details</p>
                <p className="text-xl sm:text-2xl font-black text-navy mb-1">تويوتا لاندكروزر</p>
            ...
            <div className="mb-10">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={item.id} className="bg-white/50 rounded-2xl p-4 sm:px-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0 border border-white/30 hover:bg-white transition-colors">
                    <div className="flex items-start sm:items-center gap-4">
                      <span className="text-gray-300 font-black font-sans w-6 pt-0.5 sm:pt-0">{item.id}</span>
                      <div>
                        <h4 className="font-bold text-navy text-sm sm:text-base leading-snug">{item.desc}</h4>
                        <p className="text-gray-400 text-xs font-bold font-sans tracking-wide mt-1">QTY: {item.qty} &bull; SAR {item.price}</p>
                      </div>
                    </div>
                    <div className="text-left sm:text-right w-full sm:w-auto flex justify-between sm:block border-t border-gray-100/50 sm:border-0 pt-3 sm:pt-0" dir="ltr">
                      <span className="text-[10px] text-gray-400 font-black uppercase tracking-[0.2em] block sm:mb-1">SAR</span>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={item.id} className="bg-white/50 rounded-2xl p-4 sm:px-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0 border border-white/30 hover:bg-white transition-colors">
                    ...
                    <div className="text-left sm:text-right w-full sm:w-auto flex justify-between sm:block border-t border-gray-100/50 sm:border-0 pt-3 sm:pt-0" dir="ltr">
                      <span className="text-[10px] text-gray-400 font-black uppercase tracking-[0.2em] block sm:hidden">SAR</span>
                      <span className="text-xl font-black text-navy font-sans">{item.total}</span>
                    </div>
                  </div>
                ))}
          ...
          <div className="bg-navy p-6 sm:p-10 flex flex-col md:flex-row justify-between items-end gap-10 relative shrink-0">
            ...
            <div className="w-full md:w-1/3 text-right relative z-10">
              ...
              <div className="flex justify-between items-center py-2 text-gray-300 border-b border-white/10 mb-4 pb-4">
                <span className="font-bold text-sm sm:text-base">أجور العمل والضريبة</span>
              ...
              <div className="flex justify-between items-end mb-8">
                ...
                <div dir="ltr" className="text-left">
                  <span className="text-xs sm:text-sm text-teal-light font-black uppercase tracking-[0.2em] block mb-1">SAR</span>
                  <span className="text-4xl sm:text-5xl font-black text-white font-sans tracking-tighter">1,771</span>
                </div>
              </div>
              <button className="w-full flex items-center justify-center gap-2 px-6 sm:px-8 py-3 sm:py-4 bg-white text-navy font-black rounded-2xl hover:bg-gray-100 transition-all shadow-lg hover:shadow-xl hover:-translate-y-1">
                <Download size={20} />
                <span className="text-sm sm:text-base">تحميل الفاتورة PDF</span>
              </button>
            </div>
          </div>
        </div>
